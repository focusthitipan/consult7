...
2. Authenticate with your Google account:
   ...
   - **No additional setup needed** - Consult7 will automatically use the same OAuth client as the Gemini CLI tool

## OAuth Credentials Format

Your `~/.gemini/oauth_creds.json` will look like this (generated by `gemini` CLI):
...
**Note**: The file doesn't need `client_id` or `client_secret` - Consult7 will automatically use the public OAuth client credentials from the Gemini CLI tool for token refresh.
